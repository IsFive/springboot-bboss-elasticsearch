...
public class ESUtil {
	...
	public void addOrUpdateDocument(String index,Object obj){
		...
		//向固定index添加或者修改文档,如果demoId已经存在做修改操作，否则做添加文档操作，返回处理结果
		//索引表 索引类型
	 ...
	 * @param clientUtil 客户端
	 */
	public <T> List<T> getDocumentList(String index,T params,String dsl,ClientInterface clientUtil){
		//设定查询条件,通过map传递变量参数值,key对于dsl中的变量名称
		//执行查询，index为索引表，_search为检索操作action
		//使用反射获取Class<T>
		Class<T> clazz = GenericsUtils.getSuperClassGenricType(params.getClass(), 0);
